...
최종 설명: 이미지, script, form, iframe은 웹의 역사적 리소스 포함/전송 모델 때문에 cross-origin 사용이 일부 허용된다. 하지만 SOP는 현재 origin의 JavaScript가 다른 origin의 응답 본문, DOM, storage 같은 민감한 내부 데이터를 읽거나 조작하는 것을 제한한다. `fetch()`는 응답을 JS가 읽기 위한 API이므로 CORS 허용 없이는 cross-origin 응답이 JS에 노출되지 않는다. 따라서 각 기능은 “로드/전송/실행은 가능할 수 있지만 읽기는 제한된다”는 공통 원칙 아래 서로 다르게 동작한다.
